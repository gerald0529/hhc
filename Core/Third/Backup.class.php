...
final class Backup
{
    ...
    {
        //首次执行时没有备份状态
        if (!I("get.status")) F('backupDir', null);
        //获取备份目录
    ...
    static private function backup_structure()
    {
        ...
        foreach (self::$config as $table => $config) {
            ...
            $sql .= "\$db->execute(\"DROP TABLE IF EXISTS `\".\$db_prefix.\"" . str_replace($db_prefix, '', $table) . "`\");\n";

            $sql .= "\$db->execute(\"{$create_table_sql}\");\n";
        }
    ...
    static private function backup_data()
    {
        ...
        foreach (self::$config as $table => $config) {
            ...
            C('DB_NAME', $config['database']);
            $tablename = str_replace(C('DB_PREFIX'), '', $table);
            $db = M($tablename);
            ...
            do {
                ...
                if (!$data) {
                    ...
                    foreach ($data as $d) {
                        ...
                        $backup_str .= "\$db->execute(\"REPLACE INTO $table_name (`" . implode("`,`", $field) . "`)
						VALUES(" . $dataValue . ")\");\n";
                    }
                }
                //检测本次备份是否超出分卷大小
    ...
    {
        //当前备份分卷id
        $fid = I("get.bid", 1, 'intval');
        if(!isset($_GET['bid']))
            $_GET['bid']=1;
        file_put_contents(self::$dir . "/{$table}_bk_{$fid}.php", "<?php \n{$data}");
   ...
   static public function getAllTableInfo()
    {
        $db =  M();
        $info =$db->query("SHOW TABLE STATUS FROM " . C("DB_NAME"));
        $arr = array();
        ...
        foreach ($info as $k => $t) {
            ...
            $name = str_replace(C('DB_PREFIX'), '', $t['name']);
            $tempdb = M($name);
            $fieldData = $tempdb->getDbFields();
            $arr['table'][$t['name']]['field'] = $fieldData;
            $arr['table'][$t['name']]['primaryKey'] = $tempdb->getPk();
    ...
    static private function init($config)
    {
        ...
        //所有表信息
        $tableInfo = self::getAllTableInfo();
