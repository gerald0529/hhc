...
class IndexController extends Controller {
    ...
    public function step1()
    {
    	...
    	// 需要可以读写的目录
    	$dir = array(
    		array('name'=>'./Core','text'=>'主程序','status'=>0),
    		array('name'=>'./Data','text'=>'配置文件、数据、缓存','status'=>0),
    	);
    ...
    private function _write_config()
    {
        ...
        $data['AUTH_CONFIG']= array(
            'AUTH_ON'=>true,
            'AUTH_TYPE'=>1,
            'AUTH_GROUP'=> $data['DB_PREFIX'].'auth_group',
            'AUTH_GROUP_ACCESS'=>  $data['DB_PREFIX'].'auth_group_access',
            'AUTH_RULE'=>  $data['DB_PREFIX'].'auth_rule',
            'AUTH_USER'=>  $data['DB_PREFIX'].'user',

        );
        $mtstr = rand_str();
        $data['SESSION_OPTIONS'] = array('name'=>$mtstr);
 ...
 * @Last Modified time: 2015-08-07 18:26:13
 */
...
str;
        //写入文件
        file_put_contents('./Data/Config/db.inc.php', $php);
        return true;
    }
    ...
    private function _sql_query()
    {
        ...
        // 表前缀
        $dbPrefix=I('post.dbprefix');
        $sql = $installdata?file_get_contents(APP_PATH.MODULE_NAME.'/Conf/thinkcms-data.sql'):file_get_contents(APP_PATH.MODULE_NAME.'/Conf/thinkcms.sql');
        $sql=preg_split('/;{}/', $sql);
    ...
    private function _add_user()
    {
        ...
        // 组合sql
        $sql="INSERT INTO `{$dbPrefix}user`(uid,username,password,login_ip,login_time,addtime,role,nickname,email) VALUES (1, '{$username}', '{$password}', '{$ip}', '{$time}', '{$time}', '1', '{$nickname}', '{$email}')";
